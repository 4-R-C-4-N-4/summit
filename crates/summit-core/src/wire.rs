...
pub struct HandshakeInit {
    /// Noise_XX ephemeral public key — X25519, 32 bytes.
    pub ephemeral_public: [u8; 32],

    /// The CapabilityAnnouncement the initiator is responding to, verbatim.
    /// The responder verifies capability_hash matches a locally offered capability.
    /// If it does not match, the datagram is silently dropped.
    pub announcement: [u8; 72],

    /// Cryptographically random nonce generated by the initiator.
    /// Combined with the responder's nonce to derive the session ID:
    ///   session_id = BLAKE3(initiator_nonce || responder_nonce)
    pub nonce: [u8; 16],
}

assert_eq_size!(HandshakeInit, [u8; 120]);

/// Noise_XX handshake message 2 — sent by the responder.
///
/// Contains the responder's ephemeral key and nonce, plus the Noise_XX
/// encrypted static key and encrypted handshake payload (authentication data).
/// After this message, both sides call Noise split() to derive symmetric keys
/// and the session transitions to the transport phase.
///
/// Wire size: 144 bytes.
#[derive(Debug, Clone, AsBytes, FromBytes, FromZeroes)]
#[repr(C, packed)]
pub struct HandshakeResponse {
    ...
    /// Cryptographically random nonce generated by the responder.
    /// Combined with the initiator's nonce to derive the session ID.
    pub nonce: [u8; 16],

    /// Noise_XX encrypted static key: 32 bytes key + 16 bytes Poly1305 MAC.
    pub encrypted_static: [u8; 48],

    /// Noise_XX encrypted handshake payload: 32 bytes payload + 16 bytes MAC.
    /// Currently carries the responder's negotiated schema ID list hash.
...
mod tests {
    ...
    fn handshake_init_round_trip() {
        let original = HandshakeInit {
            ephemeral_public: [0x33; 32],
            announcement:     [0x44; 72],
            nonce:            [0x55; 16],
        };
        let bytes = original.as_bytes();
        assert_eq!(bytes.len(), 120);
        let recovered = HandshakeInit::read_from(bytes).unwrap();
        assert_eq!(recovered.ephemeral_public, original.ephemeral_public);
    ...
    fn handshake_response_round_trip() {
        let original = HandshakeResponse {
            ephemeral_public:  [0x66; 32],
            nonce:             [0x77; 16],
            encrypted_static:  [0x88; 48],
            encrypted_payload: [0x99; 48],
        };
        let bytes = original.as_bytes();
        assert_eq!(bytes.len(), 144);
        let recovered = HandshakeResponse::read_from(bytes).unwrap();
        assert_eq!(recovered.nonce,             original.nonce);
        assert_eq!(recovered.encrypted_static,  original.encrypted_static);
        assert_eq!(recovered.encrypted_payload, original.encrypted_payload);
    }
